...
//!
//! 从 rust-browser/src/web_storage.rs 复制。
...
use crate::cookies::GLOBAL_COOKIE_JAR;
...
mod tests {
    ...
    fn test_local_storage() {
        ...
        storage_clear(StorageType::Local, origin);

        assert_eq!(storage_length(StorageType::Local, origin), 0);

        storage_set_item(StorageType::Local, origin, "key1", "value1");
        assert_eq!(storage_length(StorageType::Local, origin), 1);
        assert_eq!(storage_get_item(StorageType::Local, origin, "key1"), Some("value1".to_string()));

        storage_set_item(StorageType::Local, origin, "key2", "value2");
        assert_eq!(storage_length(StorageType::Local, origin), 2);

        assert_eq!(storage_key(StorageType::Local, origin, 0), Some("key1".to_string()));
        assert_eq!(storage_key(StorageType::Local, origin, 1), Some("key2".to_string()));

        storage_remove_item(StorageType::Local, origin, "key1");
        assert_eq!(storage_length(StorageType::Local, origin), 1);
        assert_eq!(storage_get_item(StorageType::Local, origin, "key1"), None);
    ...
    #[test]
    fn test_session_storage_independent() {
        let origin = "https://example.com";
        storage_clear(StorageType::Session, origin);
        storage_clear(StorageType::Local, origin);

        storage_set_item(StorageType::Session, origin, "skey", "sval");
        storage_set_item(StorageType::Local, origin, "lkey", "lval");

        assert_eq!(storage_get_item(StorageType::Session, origin, "skey"), Some("sval".to_string()));
        assert_eq!(storage_get_item(StorageType::Local, origin, "skey"), None);
        assert_eq!(storage_get_item(StorageType::Session, origin, "lkey"), None);
        assert_eq!(storage_get_item(StorageType::Local, origin, "lkey"), Some("lval".to_string()));
